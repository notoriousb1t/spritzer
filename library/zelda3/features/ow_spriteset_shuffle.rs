...
use crate::zelda3::model::{
    can_shuffle_in_ow, get_sprite_requirements, get_spritesheet_arrangements, is_list_compatible,
    is_restricted_sprite, OWRoomId, Placement, SpriteId, Spriteset, SpritesetId, SpritesheetId,
    Z3Model,
};
...
pub(crate) fn apply_ow_spriteset_shuffle(model: &mut Z3Model) {
    ...
    fill_ow_spritesets(model, &mut rng);
...
fn fill_ow_spritesets(model: &mut Z3Model, rng: &mut StdRng) {
    let spritesets = SpritesetId::iter().collect::<Vec<_>>();

    let mut spritesheet_pool = create_spritesheet_pool();
    ...
    for spriteset_id in spritesets {
        ...
        spritesheet_pool = create_spritesheet_pool();
        if fill_spriteset_from_pool(spriteset, rng, &mut spritesheet_pool) {
            continue;
        }
...
fn create_spritesheet_pool() -> Vec<[SpritesheetId; 4]> {
    SpriteId::iter()
...
fn fill_spriteset_from_pool(
    ...
    rng: &mut StdRng,
    pool: &mut Vec<[SpritesheetId; 4]>,
) -> bool {
    for _ in 0..128 {
        if let Some(spritesheets) = {
            pool.shuffle(rng);
            pool.pop()
        } {
